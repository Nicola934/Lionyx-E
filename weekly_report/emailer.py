...
def send_report_email(
    ...
    subject = f"{email_cfg.subject_prefix} Total={kpis.total_reponses}"
    body_lines = [
        "Weekly Auto-Report",
        "",
        f"Total responses: {kpis.total_reponses}",
    ]
    if kpis.total_responses:
        ...
        with smtplib.SMTP(email_cfg.smtp_host, email_cfg.smtp, timeout=20) as server:
